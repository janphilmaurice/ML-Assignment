...
def strategy_test(strategy, X, y, model, rate=(0.5,0.5)):
...
def check_imbalance(strategy=None, data=None, rates=None, model=None):
    #resample_X, resample_y = strategy(X, y)
    #num = max(resample_y[resample_y==0].shape[0],resample_y[resample_y==1].shape[0])
    accs=[]
    column = 'Binary_Credit_Score'
    for rate in rates:
        sample_data = split_data(data,column=column, rate=rate, num=5000)  # 先对预处理数据基于给定比例划分
        ...
        acc = strategy_test(strategy, X, y, model,rate)  # 再用strategy采样
